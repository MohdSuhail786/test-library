...
import { LabelList } from "../../constants/Constants";
...
export class DrawingAreaImage<Config extends DrawingAreaImageConfig = DrawingAreaImageConfig> extends Image {
    ...
    async mouseDownAction(event: KonvaEventObject<MouseEvent>) {
        try {
            ...
            if(this.editor.appMode.mode === "DRAWING_MODE") {
                ...
                const label = this.editor.labels.find(label => label.name === LabelList.DrawingArea[0]) as Label
                if(label)
    ...
    addBox(box: Box) {
        ...
        (this.editor as DrawingAreaEditor).drawingAreaLayer.add(box)
        this.syncBoxs()
    }
    ...
    syncBoxs() {
        setRecoil(entityListAtom, [...this.drawingAreas])
    }
